...
- 변경이 불가능(immutable)하며 반복이 가능(iterable)함



#### 2. 활용

- Escape Sequence : 문자열 내에서 특정 문자나 조작을 위해서 역슬래시를 활용해 구분함
...
- 키-값(key-value)의 쌍으로 이뤄진 모음 (collection)
...
- key와 value가 쌍으로 이뤄진 자료 구조이므로
  ...
  - key는 변경 불가능(immutable)한 데이터만 활용이 가능함
